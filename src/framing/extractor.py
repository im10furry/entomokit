...
class VideoFrameExtractor:
    ...
    SUPPORTED_VIDEO_FORMATS = {
        'mp4', 'mov', 'avi', 'mkv', 'webm', 'flv', 'm4v', 'mpeg', 'mpg', 'wmv', '3gp', 'ts'
    }
    ...
    def extract_frame(
        ...
        try:
            ...
            try:
                ...
                if not ret:
                    return False, f"Failed to read frame {frame_number} from {video_path.name}"
                
                if seq_num is not None:
                    output_name = f"{video_path.stem}_{seq_num:02d}.{image_format.lower()}"
                else:
                    output_name = f"{video_path.stem}_{frame_number:02d}.{image_format.lower()}"
    ...
    def extract_all(self, show_progress: bool = True) -> dict:
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            ...
            if TQDM_AVAILABLE and show_progress and tqdm is not None:
                progress = tqdm(total=len(futures), desc="Processing videos", unit="video")
