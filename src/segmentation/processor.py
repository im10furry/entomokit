...
from src.utils import load_image, save_image_rgba, apply_mask_with_alpha, save_image, get_device
...
class SegmentationProcessor:
    ...
    def __init__(
        ...
        if self.segmentation_method in ["sam3", "sam3-bbox"]:
            if sam3_checkpoint is None:
                raise ValueError("sam3_checkpoint is required for sam3/sam3-bbox methods")
    ...
    def _enlarge_bbox(self, x: int, y: int, w: int, h: int, img_w: int, img_h: int) -> Tuple[int, int, int, int]:
        """
        ...
        """
        logger.debug(f"_enlarge_bbox input: x={x}, y={y}, w={w}, h={h}, img={img_w}x{img_h}, padding_ratio={self.padding_ratio}")
    ...
    def process_image(
        ...
        if self.segmentation_method == "otsu":
            ...
            if not masks:
                ...
                return results
        elif self.segmentation_method in ["sam3", "sam3-bbox"] and self.sam_wrapper is not None:
        ...
        for mask, score in zip(masks, scores):
            if score >= self.confidence_threshold:
                ...
                logger.debug(f"Filtering mask with score {score:.3f} < {self.confidence_threshold}")
        
        masks = filtered_masks
        scores = filtered_scores
        
        if len(masks) == 0:
            logger.warning(f"No masks passed confidence threshold ({self.confidence_threshold})")
            return results
        ...
        for i, (mask, score) in enumerate(zip(filtered_masks, filtered_scores)):
            ...
            if is_bbox_mode:
                ...
                # Apply padding using reference script's enlarge_bbox logic
                img_h, img_w = mask.shape[:2]
                x, y, w, h = self._enlarge_bbox(bbox[0], bbox[1], bbox[2], bbox[3], img_w, img_h)
                ...
                cropped_rgb_masked = cropped_image[:, :, :3].astype(np.float32) * mask_float
                
                # Create RGBA with masked RGB + alpha
                if cropped_image.shape[2] == 3:
                    cropped_rgba = np.dstack([cropped_rgb_masked.astype(np.uint8), cropped_mask.astype(np.uint8) * 255])
                    ...
                    cropped_rgba = np.dstack([cropped_rgb_masked.astype(np.uint8), np.ones_like(cropped_mask) * 255])
    ...
    def _repair_with_sam3_fill(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        base_repaired = cv2.inpaint(image, dilated_mask, inpaintRadius=5, flags=cv2.INPAINT_TELEA)
        ...
        try:
            ...
            if sam_masks and len(sam_masks) > 0:
                ...
                guide_mask = cv2.resize(largest_mask.astype(np.uint8), (image.shape[1], image.shape[0]))
    ...
    def process_directory(
        ...
        for img_path in tqdm(image_paths, desc="Segmenting", disable=disable_tqdm):
            ...
            if shutdown_flag is not None and shutdown_flag():
                logger.info("Shutdown requested. Exiting after completing current image.")
                break
