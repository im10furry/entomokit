...
warnings.filterwarnings('ignore', category=RuntimeWarning, module='skimage.color.colorconv')
...
class SynthesisProcessor:
    ...
    def load_images_from_directory(self, directory: Path, desc: str = "Loading images") -> List[np.ndarray]:
        """Load all images from directory."""
        image_extensions = {'.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp'}
        image_paths = [p for p in directory.iterdir() if p.suffix.lower() in image_extensions and p.is_file()]
    ...
    def _save_image(self, image: np.ndarray, output_path: Path, quality: int = 90) -> None:
        """Save image to file."""
    ...
    def _calculate_scale_factor(self, background_shape: Tuple[int, ...], mask_area: int, scale_ratio: float) -> float:
    ...
    def _is_region_black(self, background: np.ndarray, x: int, y: int, w: int, h: int, threshold: int = 15) -> bool:
    ...
    def _random_position_with_constraint(self, background: np.ndarray, target_shape: Tuple[int, ...], edge_margin: float = 0.1) -> Tuple[int, int, float]:
        ...
        while downscale_factor >= 0.1:
            ...
            for _ in range(max_attempts):
                x = np.random.randint(min_x, max_x) if max_x > min_x else min_x
                y = np.random.randint(min_y, max_y) if max_y > min_y else min_y
                if not self._is_region_black(background, x, y, scaled_target_w, scaled_target_h):
    ...
    def _get_random_position_no_constraint(self, background: np.ndarray, target_shape: Tuple[int, ...], edge_margin: float = 0.1) -> Tuple[int, int]:
    ...
    def _paste_with_alpha(self, background: np.ndarray, target_rgba: np.ndarray, x: int, y: int) -> np.ndarray:
    ...
    def _rotate_image(self, image: np.ndarray, angle: Optional[float] = None) -> np.ndarray:
    ...
    def _calculate_max_scale_to_fit(self, target_shape: Tuple[int, ...], background_shape: Tuple[int, ...]) -> float:
    ...
    def _match_lab_histograms(self, image: np.ndarray, reference: np.ndarray) -> np.ndarray:
        ...
        reference_lab = np.nan_to_num(reference_lab, nan=0.0, posinf=100.0, neginf=-100.0)
        ...
        for channel in range(3):
            ...
            result_lab[:, :, channel] = ((1 - self.color_match_strength) * image_channel + self.color_match_strength * matched)
    ...
    def synthesize_single(self, target_image: np.ndarray, background: np.ndarray, scale_ratio: Optional[float] = None, target_path: Optional[Path] = None, counter: int = 0) -> Tuple[Optional[np.ndarray], Optional[str], float, Optional[float], Path, np.ndarray, int, int]:
        ...
        try:
            ...
            scale_factor = self._calculate_scale_factor(background.shape, mask_area, scale_ratio)
            ...
            downscale_factor = 1.0
            while (target_rotated.shape[0] > background.shape[0] or 
            ...
            if self.avoid_black_regions:
                ...
                for scale_idx, scale_factor in enumerate(scale_factors):
                    ...
                    if scale_factor < 1.0:
                        final_target_for_check = cv2.resize(original_target, (current_w, current_h), interpolation=cv2.INTER_LINEAR)
                    else:
                    ...
                    for attempt in range(max_attempts):
                        x, y = self._get_random_position_no_constraint(background, (current_h, current_w, 4), edge_margin=0.05)
                        ...
                        if region.shape[0] > 0 and region.shape[1] > 0:
                            ...
                            if black_ratio == 0:
                                ...
                                logger.debug(f"Found perfect position at scale {scale_factor:.2f} after {attempt+1} attempts")
                                break
                ...
                if not found_valid_position:
                    if best_position is not None:
                        x, y = best_position
                        logger.debug(f"Using best position with {best_black_ratio*100:.1f}% black pixels")
                    else:
                        x, y = self._get_random_position_no_constraint(background, final_target_for_check.shape, edge_margin=0.05)
            else:
                x, y = self._get_random_position_no_constraint(background, final_target_for_check.shape, edge_margin=0.05)
    ...
    def process_directory(self, target_dir: Path, background_dir: Path, output_dir: Path, num_syntheses: int = 10, disable_tqdm: bool = False, threads: int = 1) -> dict:
        ...
        image_extensions = {'.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp'}
        target_paths = [p for p in target_dir.iterdir() if p.suffix.lower() in image_extensions and p.is_file()]
        background_paths = [p for p in background_dir.iterdir() if p.suffix.lower() in image_extensions and p.is_file()]
        ...
        for target_path in target_paths:
            ...
            for syn_idx in range(num_syntheses):
                ...
                scale_ratio = random.uniform(self.area_ratio_min, self.area_ratio_max)
                tasks.append((target_img, background, scale_ratio, target_path, syn_idx + 1, background_path))
        ...
        return {'processed': synthesis_id, 'failed': total_syntheses - synthesis_id - skipped_images, 'output_files': synthesis_id, 'skipped': skipped_images}
